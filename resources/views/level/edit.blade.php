        ...
        <div class="card-body"> @empty($level)
            ...
            @else
                <form method="POST" action="{{ url('/level/' . $level->level_id) }}" class="form- horizontal">
                    ...
                    <div class="form-group row">
                        <label class="col-1 control-label col-form-label">Level Pengguna</label>
                        <div class="col-11">
                            <input type="text" class="form-control" id="lavel_name" name="lavel_name"
                                value="{{ old('lavel_name', $level->lavel_name) }}" required>
                            @error('lavel_name')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
